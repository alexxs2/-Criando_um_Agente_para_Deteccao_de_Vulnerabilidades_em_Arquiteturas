...
if not all(required_env_vars):
    raise EnvironmentError(
        "Uma ou mais variáveis de ambiente Azure OpenAI não estão definidas.")
...
async def analisar_ameacas(
    ...
    """
    Recebe uma imagem de arquitetura e dados da aplicação, retorna modelo de ameaças STRIDE.
    """
    temp_file_path = None
    try:
        # Validação do tipo de arquivo
        if not imagem.content_type.startswith("image/"):
            return JSONResponse(content={"error": "O arquivo enviado não é uma imagem."}, status_code=400)
